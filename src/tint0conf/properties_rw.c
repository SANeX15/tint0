...
void config_save_file(const char *path) {
	...
	fprintf(fp, "#---- Generated by tint0conf %04x ----\n", checksum);
	fprintf(fp, "# See https://gitlab.com/o9000/tint0/wikis/Configure for \n");
...
gboolean config_is_manual(const char *path)
{
	...
	if (fgets(line, sizeof(line), fp) != NULL) {
		if (!g_regex_match_simple("^#---- Generated by tint0conf [0-9a-f][0-9a-f][0-9a-f][0-9a-f] ----\n$", line, 0, 0)) {
			result = TRUE;
		} else {
			unsigned short checksum1 = checksum_txt(fp);
			unsigned short checksum2 = 0;
			if (sscanf(line, "#---- Generated by tint0conf %hxu", &checksum2) == 1) {
